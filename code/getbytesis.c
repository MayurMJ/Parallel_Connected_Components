...
{    
		...
		// Recieve
                ...
                //B = malloc(sizeof(int) * n);
		//MPI_Recv(B,n,MPI_INT,2,0,MPI_COMM_WORLD,MPI_STATUS_IGNOREstatus);
 		MPI_Get_count(&status, MPI_BYTE, &msg_size);
  		bytes_recv += msg_size;
		//for(i = ll; i<=lm; i++)
		//{
 		...
 		MPI_Get_count(&status, MPI_BYTE, &msg_size);
  		bytes_recv += msg_size;
			...
			// Recieve
                	//P_BS(A,ll,lm,rm+1,rh,n);
               	 	//C = malloc(sizeof(int) * n);
			//MPI_Recv(C,n,MPI_INT,(2 * my_id),0,MPI_COMM_WORLD,MPI_STATUS_IGNOREstatus);
 		MPI_Get_count(&status, MPI_BYTE, &msg_size);
  		bytes_recv += msg_size;
			//for(i = ll; i<=lm; i++)
			//{
...
void R_BS(int *A,int l ,int h,int n)
{
        int temp, m, i,j, recvcount, sendcount, key, msg_size;
	int *B;
	if(my_id == 0) 
	{
		if((h-l + 1) <= COUNT/2)
